...
async def add_order(body: OutsideOrderCreate, db: Prisma = Depends(get_db), _=Depends(ALL_ROLES)):
    return await create_order(db, body)


@router.get("/{order_id}", response_model=OutsideOrderResponse)
async def get_order_detail(order_id: str, db: Prisma = Depends(get_db), _=Depends(ALL_ROLES)):
    return await get_order(db, order_id)


@router.patch("/{order_id}", response_model=OutsideOrderResponse)
async def update_order_endpoint(
    order_id: str, body: OutsideOrderUpdate, db: Prisma = Depends(get_db), _=Depends(ALL_ROLES)
):
...
async def delete_order_endpoint(
    order_id: str, db: Prisma = Depends(get_db), _=Depends(CEO_DIRECTOR)
):
