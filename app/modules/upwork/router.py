...
async def add_profile(body: UpworkProfileCreate, db: Prisma = Depends(get_db), _=Depends(CEO_DIRECTOR)):
...
async def add_snapshot(body: UpworkSnapshotCreate, db: Prisma = Depends(get_db), _=Depends(ALL_ROLES)):
    return await create_snapshot(db, body)


@router.get("/profiles/{profile_id}/snapshots", response_model=list[UpworkSnapshotResponse])
...
async def add_order_entry(body: UpworkOrderCreate, db: Prisma = Depends(get_db), _=Depends(ALL_ROLES)):
    return await add_order(db, body)


@router.get("/profiles/{profile_id}/orders", response_model=list[UpworkOrderResponse])
